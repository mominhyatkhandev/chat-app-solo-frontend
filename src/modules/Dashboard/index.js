...
const Dashboard = () => {
  ...
  useEffect(() => {
  	setSocket(io(process.env.REACT_APP_SOCKET_PORT || 'http://localhost:8080'));
  }, [])
  ...
  useEffect(() => {
    ...
    const fetchConversations = async () => {
      const res = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/conversation/${loggedInUser?._id}`,
        {
          method: "GET",
          headers: {
  ...
  const fetchMessages = async (conversationId, receiver) => {
    const res = await fetch(
      `${process.env.REACT_APP_BACKEND_URL}/api/message/${conversationId}?senderId=${user?._id}&&receiverId=${receiver?.receiverId}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
      ...
      <div className="w-[50%] h-screen bg-white flex flex-col items-center">
        ...
        <div className="h-[75%] w-full overflow-scroll overflow-x-hidden shadow-sm">
          <div className="p-14">
            {messages?.messages?.length > 0 ? (
              ...
              <div className="text-center text-lg font-semibold mt-24">
                No Messages! Select a conversation to start chatting!
              </div>
            )}
          </div>
        </div>
        {messages?.receiver?.fullName && (
          <div className="p-14 w-full flex items-center">
            ...
            />
            <div
              className={`ml-4 p-2 cursor-pointer bg-primary rounded-full ${
                !message && "pointer-events-none bg-slate-100"
              }`}
              onClick={() => sendMessage()}
            >
              <svg
